...
//  4) create 'whenNotPaused' & 'paused' modifier that throws in the appropriate situation
    modifier whenNotPaused(){
        require(!_paused, "Contract must be paused to proceed");
        _;
    }

    modifier paused(){
        require(_paused, "Contract must be resumed to proceed");
        _;
    }

//  5) create a Paused & Unpaused event that emits the address that triggered the event
    event Paused(address source);
    event Unpaused(address source);
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId), "Could not approve owner");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(isOwner(msg.sender) || isApprovedForAll(getOwner(), to), "Caller is not approved");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId) == from, "Transfer can only be made by the owner");

        // TODO: require token is being transfered to valid address
        require(to != address(0), "Address is not valid, could not transfer token");
